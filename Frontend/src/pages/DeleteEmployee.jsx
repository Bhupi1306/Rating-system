...
const DeleteEmployee = () => {
  ...
  {
    try {
      const url = "http://localhost:8000/employee/delete"
  ...
  const handleSubmit = async (e) => {
    ...
    try{
      const url = "http://localhost:8000/employee/check"
      const response = await fetch(url, {
              method: "POST",
              headers: {
                'content-type':'application/json'
              ...
              <div className="flex min-h-full flex-1 flex-col justify-center px-6 py-12 lg:px-8 mt-18 relative ">
                ...
                <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
                  <form onSubmit={handleSubmit}  className="space-y-6">
                    ...
                    <div>
                        <input type="Text" value={id} onChange={handleChange} placeholder="Employee ID" className="rounded-md w-2xs bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6" />
